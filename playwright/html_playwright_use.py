...
def doubao_parse_html_to_string(html_content: str) -> str:
    try:
        ...
        containers = soup.find_all("div", class_="container-jJBCG1") or soup.find_all(
            "div", class_="zone-container editor-kit-container"
        )

        if not containers:
            # 直接处理所有相关元素
            all_elements = soup.find_all(["div", "h1", "h2", "h3", "h4", "h5", "h6", "ul", "ol", "li"])

            for element in all_elements:
                # 处理段落元素
                if element.name == "div" and "paragraph-element" in element.get("class", []):
                    text_content = doubao_process_paragraph(element)
                    if text_content:
                        result.append(text_content)

                # 处理标题元素
                elif element.name in ["h1", "h2", "h3", "h4", "h5", "h6"]:
                    if "header-" in " ".join(element.get("class", [])):
                        text = element.get_text(strip=True)
                        if text:
                            result.append(f"## {text}")

                # 处理列表元素
                elif element.name == "ul" and "auto-hide-last-sibling-br" in element.get("class", []):
                    # 确保这个ul不是其他ul的子元素
                    if not element.find_parent("ul"):
                        list_content = process_list_with_nested(element)
                        if list_content:
                            result.extend(list_content)
            ...
            if not result:
                # 移除引用标记和其他干扰元素
                for ref in soup.find_all("span", class_="ref_content_circle"):
                    ref.decompose()
        ...
        return "\n\n".join(result) if result else "未找到有效内容"
